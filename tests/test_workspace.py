...
def test_workspace_creates_versioned_schema_and_only_the_parent(tmp_path: Path):
    ...
    assert tables == {
        ...
        "reference_expansion_runs",
        "reference_expansion_nodes",
        "reference_expansion_edges",
        "reference_expansion_attempts",
        "reference_expansion_events",
        "reference_expansion_lease",
        }
    assert fetch_all(
        path,
...
def test_workspace_rejects_newer_schema_and_closes_connection(
    ...
    with sqlite3.connect(path) as connection:
        ...
        connection.execute(
            "INSERT INTO workspace_meta VALUES ('schema_version', '10')"
        )
    ...
    with pytest.raises(WorkspaceSchemaError, match="schema version 10"):
        Workspace.open(path)
