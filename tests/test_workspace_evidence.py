...
def test_latex_import_populates_proof_roadmap_dependencies(workspace, tmp_path: Path):
    main = tmp_path / "main.tex"
    main.write_text(
        "\n".join(
            [
    ...
    assert {
        mention["method"]
        for result in dependencies["known"]["resolved_local_results"]
        for mention in result["via_mentions"]
    } == {"proof_roadmap_result_regex"}


def test_latex_import_dedupes_roadmap_and_label_dependency(workspace, tmp_path: Path):
    main = tmp_path / "main.tex"
    main.write_text(
        "\n".join(
            [
                ...
                r"\end{theorem}",
                r"\begin{proof}",
                r"We use \cref{lem:base}. It remains to prove Lemma 1.",
                r"\end{proof}",
                r"\end{document}",
            ]
        ),
    ...
    assert [
        mention["method"]
        for mention in dependencies["known"]["resolved_local_results"][0]["via_mentions"]
    ] == ["proof_latex_ref"]
