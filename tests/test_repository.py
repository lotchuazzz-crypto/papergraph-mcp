...
def test_runtime_and_issue_template_release_strings_remain_pinned():
    ...
    assert f'PaperGraph/1.1.1 (+{REPOSITORY_URL})' in arxiv_source
    assert version_field["attributes"]["placeholder"] == "1.1.1"
...
def test_ci_workflow_is_cross_platform_locked_and_least_privilege():
    ...
    assert "papergraph-mcp 1.1.1" in build_steps
...
def test_readme_is_a_version_pinned_launch_page_with_verified_demo():
    ...
    assert "PaperGraph v1.1.1" in readme
    assert (
        "PaperGraph v1.1.1 is the stable evidence-first reading workflow"
        in readme
    )
    ...
    assert "v1.1.2 should focus on Reference Resolution" in readme
...
def test_onboarding_uses_v061_release_pin_and_mentions_id_url_conflicts():
    ...
    assert "papergraph-mcp.git@v1.1.1" in skill
    assert "papergraph-mcp 1.1.1" in skill
